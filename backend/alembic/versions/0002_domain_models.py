...
def upgrade() -> None:
    player_position_enum = postgresql.ENUM("GK", "DEF", "MID", "FWD", name="player_position", create_type=True)
    ...
    op.create_table(
        ...
        sa.Column("position", postgresql.ENUM("GK", "DEF", "MID", "FWD", name="player_position", create_type=False), nullable=False),
    ...
    op.create_table(
        "matches",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("home_team_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("teams.id", ondelete="RESTRICT"), nullable=False),
        sa.Column("away_team_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("teams.id", ondelete="RESTRICT"), nullable=False),
        sa.Column("kickoff_at", sa.DateTime(timezone=True), nullable=False),
        sa.Column("venue", sa.String(200), nullable=True),
        sa.Column("status", postgresql.ENUM("SCHEDULED", "LIVE", "FINISHED", "CANCELLED", "POSTPONED", name="match_status", create_type=False), nullable=False, server_default="SCHEDULED"),
